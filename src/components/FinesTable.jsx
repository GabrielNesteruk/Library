...
const FinesTable = ({
  ...
  const getFines = async () => {
    const activeFines = await fetchFines();
    setMemberFines(activeFines);
  };

  function postPayment(id, payment) {
    axios
      .post("http://localhost:8081/api/fines/" + id + "/payments", payment)
      .then((response) => {
            ...
            (fine) =>
              fine && (
                <tr key={fine.id}>
                  ...
                  {userFines && fine.finePayments.length === 0 ? (
                    <td>
                      <button
                        onClick={(e) => payFine(e, fine.id, fine.fineAmount)}
                        style={buttonStyle}
                      >
                        Opłać
                      </button>
