...
const Account = ({ setIsLogged }) => {
  ...
  const fetchLoans = async () => {
    const res = await fetch(
      "http://localhost:8081/api/loans/members/" + userData.id,
      {
        headers: {
  ...
  const fetchFines = async () => {
    const res = await fetch(
      "http://localhost:8081/api/fines/members/" + userData.username,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + localStorage.getItem("token"),
  ...
  return (
    <div className="account-container">
      ...
      <div className="right-side">
        ...
        >
          <LoansTable
            memberActiveLoansList={memberActiveLoansList}
            deleteLoan={false}
          />
        </div>
        <div
          className="fines"
          style={{ display: fineClicked === true ? "flex" : "none" }}
        >
          <FinesTable
            memberActiveFinesList={memberFines}
            setMemberFines={setMemberFines}
            userFines={true}
